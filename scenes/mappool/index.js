...
class Beatmap {
  ...
  generate() {
    this.parent = $('<div></div>')
      .addClass('map')
      .attr('id', `map-${this.beatmap.identifier.toLowerCase()}`);
    const image = $('<div></div>')
      .addClass('map-image')
      .css(
        'background-image',
        `url('https://assets.ppy.sh/beatmaps/${this.beatmap.beatmapset_id}/covers/cover.jpg?1')`
      );
    this.parent.append(
      $('<div></div>')
        .addClass('map-image-container')
        .append(
          $('<div></div>').addClass('map-image-container-inside').append(image)
        )
    );

    const content = $('<div></div>').addClass('map-content');
    this.picked_by_label = $('<div></div>')
      .addClass('picked-by-label')
      .attr('id', `picked-by-label-${this.beatmap.identifier.toLowerCase()}`);
    content.append(this.picked_by_label);
    this.mod_icon = $('<div></div>')
      .addClass(`mod-icon ${this.beatmap.mods.toLowerCase()}`)
      .text(this.beatmap.identifier.toUpperCase());
    content.append(this.mod_icon);

    const stats = $('<div></div>').addClass('map-stats');
    stats.append(
      $('<div></div>')
        .addClass('map-stats-section map-top')
        .append(
          $('<div></div>')
            .addClass('map-title')
    ...
    bottom.append(
      $('<div></div>')
        .addClass('map-difficulty-container')
        .append(
          $('<div></div>')
    ...
    if (this.beatmap.custom) {
      this.parent.append(
        $('<div></div>')
          ...
          .text(this.beatmap.original ? 'ORIGINAL!' : 'CUSTOM!')
      );
    }
...
socket.onmessage = async (event) => {
  ...
  if (teams && regions && cache.nameRed !== data.tourney.team.left) {
    ...
    document.querySelector(':root').style.setProperty(`--red`, region?.color ?? 'var(--accent-mod-dt)');
  }
  ...
  if (teams && regions && cache.nameBlue !== data.tourney.team.right) {
    ...
    const region = regions.find((r) => r.name === team?.region);
    document.querySelector(':root').style.setProperty(`--blue`, region?.color ?? 'var(--accent-mod-dt)');
  }
  ...
  if (
    ...
    if (!cache.beatmap_id) cache.beatmap_id = data.beatmap.id;
      ...
      cache.beatmap_id = data.beatmap.id;
      const pickedMap = Array.from(beatmaps).find(
        (b) => b.id === cache.beatmap_id
      );
      if (
        pickedMap &&
        toggles.enableAutoPick &&
        !selectedMaps.includes(cache.beatmap_id)
      )
        pickMap(
          pickedMap,
...
const transitionToMappool = async (data) => {
  ...
  if (toggles.enableAutoAdvance) {
    if (
      cache.lastState === TourneyState.Ranking &&
      newState === TourneyState.Idle
    ) {
...
const getDataSet = (stored_beatmaps, beatmap_id) =>
  stored_beatmaps.find((b) => b.beatmap_id == beatmap_id) || null;
...
const pickMap = (bm, teamName, color) => {
  ...
  if (bm.beatmap.mods.includes('TB')) {
    ...
    bm.picked_by_label
      .text('Tiebreaker')
      .addClass(`picked`)
      .removeClass('banned red blue');
  } else {
    localStorage.setItem('current_pick', `${bm.id}/${color.toLowerCase()}`);
    bm.parent
      .addClass(`picked ${color}`)
      .removeClass(`banned ${opposite_team(color)}`);
    bm.picked_by_label
      .text(`Picked by ${teamName}`)
      .addClass(`picked ${color}`)
      .removeClass(`banned ${opposite_team(color)}`);
  }
  ...
  if (toggles.enableAutoAdvance) {
    ...
    if (selectedMapsTransitionTimeout[bm.id]?.color !== color) {
      ...
      autoadvance_timer_time = new CountUp(
        'autoAdvanceTimerTime',
...
const banMap = (bm, teamName, color) => {
  ...
  bm.parent
    .addClass(`banned ${color}`)
    .removeClass(`picked ${opposite_team(color)}`);
  bm.picked_by_label
    .text(`Banned by ${teamName}`)
    .addClass(`banned ${color}`)
    .removeClass(`picked ${opposite_team(color)}`);
...
const switchPick = (color) => {
  cache.currentPicker = color
    ? opposite_team(color)
    : opposite_team(cache.currentPicker);
  $('#current_pick')
    .text(`${cache.currentPicker.toUpperCase()} PICK`)
    .addClass(cache.currentPicker)
    .removeClass(opposite_team(cache.currentPicker));
};
